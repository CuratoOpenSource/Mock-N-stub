public protocol CanStub: HasCallValues {
    ...
    func given(_ method: String, withArgumentsThatMatch matcher: @escaping ArgumentMatcher, willReturn value: Any?)
...
public extension CanStub {
    ...
    func value(for selector: Selector, with arguments: Any) -> Any? {
        return callValues.filter({ $0.selector == selector }).filter({ $0.matcher(arguments) }).compactMap({ $0.value }).first
    }
    
    func value(for method: String, with arguments: Any) -> Any? {
        return callValues.filter({ $0.function == method }).filter({ $0.matcher(arguments) }).compactMap({ $0.value }).first
    }
}
