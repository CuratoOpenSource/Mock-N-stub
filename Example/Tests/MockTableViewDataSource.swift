...
class MockTableViewDataSource: NSObject, CanMock, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        return value(for: #selector(tableView(_:numberOfRowsInSection:)), with: (tableView, section)) ?? 0
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        didCall(#selector(tableView(_:cellForRowAt:)), with: (tableView, indexPath))
        return value(for: #selector(tableView(_:cellForRowAt:)), with: (indexPath)) ?? UITableViewCell()
    }
}
